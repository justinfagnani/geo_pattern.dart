...
class GeoPattern {
  ...
  static const FILL_COLOR_DARK = "#222";
  static const FILL_COLOR_LIGHT = "#ddd";
  static const STROKE_COLOR     = "#000";
  static const STROKE_OPACITY   = 0.02;
  static const OPACITY_MIN      = 0.02;
  static const OPACITY_MAX      = 0.15;
//
...
//
  ...
  GeoPattern(String string, [Map opts])
      : opts = {}..addAll(DEFAULTS),
  ...
  static String hexDigest(String s) {
    var bytes = (new SHA1()..add(s.runes.toList())).close();
    return CryptoUtils.bytesToHex(bytes);
  }

  String get uri_image => "url(data:image/svg+xml;base64,${base64_string});";

  String get base64_string => window.btoa(svg.innerHtml);

  generateBackground() {
    var rgb;
    if (opts.containsKey(#color)) {
      rgb = new Color.fromHex(opts[#color]);
    } else {
      var hue_offset = map(hex_val(14, 3), 0, 4095, 0.0, 1.0);
      var sat_offset = map(hex_val(17, 1), 0, 16, 0.0, 1.0);
      assert(sat_offset <= 1.0);
      assert(sat_offset >= 0.0);

      HslColor base_color = new RgbColor.fromHex(opts[#base_color]).toHsl();
      base_color = base_color.withHue((base_color.hue - hue_offset) % 1.0);

      if (sat_offset % 2 == 0) {
        base_color = base_color.withSaturation((base_color.saturation + sat_offset) % 1.0);
      } else {
        base_color = base_color.withSaturation((base_color.saturation - sat_offset) % 1.0);
      }
      rgb = base_color.toRgb();
    }
    var r = rgb.r; //(rgb.r * 255).toInt();
    var g = rgb.g; //(rgb.g * 255).toInt();
    var b = rgb.b; //(rgb.b * 255).toInt();
    svg.children.add(new RectElement()..attributes = {
      'x': '0',
      'y': '0',
      'width': '100%',
      'height': '100%',
  ...
  generatePattern() {
    if (opts.containsKey(#generator)) {
        if (PATTERNS.contains(opts[#generator])) {
          // chooses which generator
          switch (opts[#generator]) {
            case "octogons":
              geo_octogons();
              break;
            default:
              throw new ArgumentError("Error: the requested generator is invalid.");
//            "overlapping_circles",
//            "plus_signs",
//            "xes",
//            "sine_waves",
...
//        send("geo_#{PATTERNS[pattern]}")
  ...
  int hex_val(int index, int length) => int.parse(hash.substring(index, index + length), radix: 16);
...
//
  geo_octogons() {
    var square_size = map(hex_val(0, 1), 0, 15, 10, 60);
    var tile = build_octogon_shape(square_size);

    svg.attributes['width'] = '${square_size * 6}';
    svg.attributes['height'] = '${square_size * 6}';

    var i = 0;
    for (var y in range(0, 6)) {
      for (var x in range(0, 6)) {
        var val = hex_val(i, 1);
        var o = opacity(val);
        var fill = fill_color(val);

        var line = new PolylineElement()..attributes = <String, String>{
          "points": tile,
          "fill": fill,
          "fill-opacity": '$o',
          "stroke": STROKE_COLOR,
...
//    end
//
    build_octogon_shape(square_size) {
      var s = square_size;
      var c = s * 0.33;
      return "${c},0,${s-c},0,${s},${c},${s},${s-c},${s-c},${s},${c},${s},0,${s-c},0,${c},${c},0";
    }
//
...
// Dart implementation of Processing's map function
// http://processing.org/reference/map_.html
double map(num value, num min1, num max1, num min2, num max2) {
